...
$gmail=$_SESSION['email'];
    ...
    $results_per_page = 4;  
    $query = "SELECT * FROM `tbl_registration` where email='$gmail';";  
    ...
    $query = "SELECT * FROM tbl_registration  where email='$gmail' LIMIT " . $page_first_result . ',' . $results_per_page;  
...
<?php 
    $sql2="SELECT * FROM `tbl_paymant` where user_gmail='$gmail'";
    ...
    while ($row=mysqli_fetch_assoc($run) and $row2=mysqli_fetch_assoc($qry2)) 
    {
        $id=$row['id'];
                                  echo'<tr>
                                        ...
                                        <a href="../admin/payment_view/payment_recipt.php?gmail='.$gmail.'&fees='.$row['fees'].'" class="uil uil-arrow-circle-down" aria-hidden="true" style=" font-size:45px;color: #06A3DA;position:relative;left:-40px;"></a>
