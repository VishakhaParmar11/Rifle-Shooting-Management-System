...
<?php
            ...
            $id=$_GET['id'];
            $sql="SELECT * FROM `tbl_gun` where id=$id";
            $qry=mysqli_query($con,$sql);
            $row=mysqli_fetch_assoc($qry);

            $title_of_gun=$row['title_of_gun'];
            $gun_desc=$row['gun_desc'];
            $post_date=$row['post_date'];
            $gun_photo=$row['gun_photo'];
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="position:relative; left:595px;">
                                        <ul class="breadcome-menu">
                                            <h3 style="position:relative; color: #06A3DA; font-family: centuary; left: 70px;">Update Guns Information</h3>
                                        </ul>
                                    </div>
                                </div>
                            </div>
            ...
            document.getElementById('input_file').click();
                                                            ...
                                                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="position:relative; top:15px;">
                                                                <div class="form-group" style="position: relative; border-radius: 30px;">
                                                                    <input name="gun_name"value="<?php echo $title_of_gun; ?>" type="" class="form-control" placeholder="Title Of Gun" style="border: 2px solid #06A3DA; border-radius: 10px;">
                                                                </div>
                                                                
                                                                <div class="form-group res-mg-t-15">
                                                            <textarea name="gun_desc" placeholder="Description" style="border: 2px solid #06A3DA; border-radius:10px;position: relative;height: 150px;"><?php echo $gun_desc; ?></textarea>
                                                                </div>
...
<?php
    ...
    if(isset($_POST['update']))
    {
        ...
        $gun_desc=$_POST['gun_desc'];
        $date = date('m/d/Y', time());
        $f_name=$_FILES['photo']['tmp_name'];
        $path="image/gun_photo/".$_FILES['photo']['name'];
        
        if (move_uploaded_file($f_name,$path)) 
        {
            $sql1="UPDATE `tbl_gun` SET `title_of_gun` = '$gun_name', `gun_desc` = '$gun_desc', `post_date` = '$date', `gun_photo` = '$path' where id=$id";
        }
        else 
        {
            $sql1="UPDATE `tbl_gun` SET `title_of_gun` = '$gun_name', `gun_desc` = '$gun_desc', `post_date` = '$date'where id=$id";
        }
        ...
        if ($qry1) 
        {
            ...
            echo "<script>window.location.href='../../../../RSMS_project/admin/manage_gun.php?status=Guns';</script>";
        }
    }
